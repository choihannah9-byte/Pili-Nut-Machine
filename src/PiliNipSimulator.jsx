...
// ---------- engineering data (mm) — RECONCILED (panel review 2026-08) ----------
// Rev B's sim used fixed representative nuts with hand-typed "recommended"
// windows that contradicted its own outcome rule (small @ 12–13 mm and
// medium @ 16–17 mm crushed in the sim's own judge()). C1 fixes this by
// DERIVING the window live from the two gap inequalities in the thesis:
//   gap ≥ largest in-grade kernel + clearance      (kernel protection)
//   gap ≤ smallest in-grade nut width − fracture deformation
// so the sim can never disagree with its own physics. All dimensions are
// PLACEHOLDERS pending the team's own 300-nut survey and dissection data.
const DEFORM = 1.1;  // mm transverse fracture deformation [Gallegos 2013 — verify on own lot]
const CLEAR = 0.5;   // mm kernel clearance margin [provisional]
const KFRAC = 0.54;  // kernel Ø ≈ 0.54 × nut width [placeholder — from dissection]
const KJIT = 0.4;    // ± scatter on the kernel correlation, mm
const GRADES = {
  small: { label: "Small", w: [20, 25], shell: "#6b4a2e" },   // deck 20–25 mm
  medium: { label: "Medium", w: [25, 31], shell: "#5d3f26" }, // deck 25–31 mm
  large: { label: "Large", w: [31, 36], shell: "#4f351f" },   // deck 31 mm – max nut
};
const ROLLER_R = 75; // Ø150
const FACE_W = 80;

const kernelMax = (g) => KFRAC * g.w[1] + KJIT;
function recWindow(g) {
  // the two inequalities, rounded inward, clamped to the 10–27 mm screw range
  return [
    Math.max(10, Math.ceil(kernelMax(g) + CLEAR)),
    Math.min(27, Math.floor(g.w[0] - DEFORM)),
  ];
}
function sampleNut(g) {
  const width = g.w[0] + Math.random() * (g.w[1] - g.w[0]);
  const kernel = KFRAC * width + (Math.random() - 0.5) * 2 * KJIT;
  return { width: Math.round(width * 10) / 10, kernel: Math.round(kernel * 10) / 10 };
}
// per-nut outcome — the SAME two inequalities applied to the sampled nut
function judge(gap, nut) {
  if (nut.width - gap < DEFORM) return "uncracked";
  if (gap < nut.kernel + CLEAR) return "crushed";
  return "whole";
}

const OUTCOME_META = {
  whole: { stamp: "WHOLE KERNEL", color: "#7fc98f", note: "shell fractured, kernel clears the hard stop" },
  crushed: { stamp: "KERNEL CRUSHED", color: "#e05b4b", note: "hard stop below this nut's kernel Ø + clearance" },
  uncracked: { stamp: "UNCRACKED", color: "#d9a53c", note: "squeeze under the ~1.1 mm fracture deformation" },
};
...
export default function PiliNipSimulator() {
  ...
  const pushResult = useCallback((outcome, dims) => {
    setTally((t) => ({ ...t, [outcome]: t[outcome] + 1 }));
    setLog((l) => [{ outcome, gap: live.current.gap, grade: live.current.grade, dims, id: Date.now() }, ...l].slice(0, 5));
  }, []);
  ...
  useEffect(() => {
    ...
    S.feed = () => {
      ...
      const dims = sampleNut(g);                     // C1: every nut is an individual
      const nut = triPrism(dims.width, dims.width * 1.7, g.shell);
      nut.rotation.x = Math.PI / 2; // long axis along roller face (z)
      nut.position.set(150, 235, 0);
      scene.add(nut);
      S.nut = nut; S.phase = "channel"; S.t = 0;
      S.dims = dims;
      S.outcome = judge(live.current.gap, dims);
      S.grade = g;
      return true;
    };

    S.spawnFrags = (crushKernel) => {
      const g = S.grade, dims = S.dims;
      // shell halves — deflected toward shell bin (left)
      for (let i = 0; i < 3; i++) {
        const f = triPrism(dims.width * (0.45 + Math.random() * 0.2), dims.width * 0.7, g.shell);
      ...
      // kernel
      const kGeo = new THREE.SphereGeometry(S.dims.kernel / 2, 20, 16);
      kGeo.scale(1, 1.5, 1);
    ...
    const animate = () => {
      ...
      if (S.nut) {
        ...
        if (S.phase === "channel") {
          ...
          n.position.y -= 140 * dt;
          const contactY = Math.sqrt(Math.max(1, (ROLLER_R + S.dims.width / 2) ** 2 - cx ** 2));
          ...
          n.position.y -= v * dt;
          const squeeze = Math.max(gapNow / S.dims.width, 0.55);
          ...
          if (n.position.y <= -60) {
            if (S.outcome === "uncracked") {
              ...
              S.done && S.done(S.outcome, S.dims);
            }
          }
          ...
          if (n.position.y < -200) {
            ...
            S.done && S.done("uncracked", S.dims);
          }
        }
      }
  ...
  useEffect(() => {
    stateRef.current.done = (outcome, dims) => { pushResult(outcome, dims); setBusy(false); };
  }, [pushResult]);
  ...
  const rec = recWindow(g);
  const inRec = gap >= rec[0] && gap <= rec[1];
  const predicted = gap < rec[0] ? "crushed" : gap > rec[1] ? "uncracked" : "whole";
  const predictNote =
    gap < rec[0]
      ? `crush risk — gap under max in-grade kernel Ø (${kernelMax(g).toFixed(1)}) + ${CLEAR} clearance`
      : gap > rec[1]
        ? `uncracked risk — smallest in-grade nuts (${g.w[0]} mm) get < ${DEFORM} mm squeeze`
        : `inside the derived window: every sampled nut fractures and every kernel clears the stop`;
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: inRec ? "#7fc98f" : "#d9a53c", marginTop: 4 }}>
            {inRec ? `within the DERIVED window for ${g.label.toLowerCase()} grade (${rec[0]}–${rec[1]} mm)`
              : `derived window for ${g.label.toLowerCase()} grade: ${rec[0]}–${rec[1]} mm`}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "#5d6d7c", marginTop: 6 }}>
            widths {g.w[0]}–{g.w[1]} mm · kernels ≈ Ø{(KFRAC * g.w[0]).toFixed(1)}–{kernelMax(g).toFixed(1)} mm [placeholder]
          </div>
          <div style={{ fontSize: 10.5, color: "#8fa1b3", marginTop: 6, lineHeight: 1.5, border: "1px dashed #3a4a5a", borderRadius: 4, padding: "6px 8px" }}>
            window = [ max kernel + {CLEAR} , min width − {DEFORM} ] → <b style={{ color: "#f2913d" }}>{rec[0]}–{rec[1]} mm</b>. Each fed nut is sampled from the grade, so results scatter like a real lot.
          </div>
        </div>
        ...
        <div style={{ border: `1px dashed ${OUTCOME_META[predicted].color}`, borderRadius: 4, padding: "10px 12px" }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: OUTCOME_META[predicted].color, margin: "4px 0 2px" }}>
            {predicted === "whole" ? "WHOLE-KERNEL WINDOW" : OUTCOME_META[predicted].stamp + " RISK"}
          </div>
          <div style={{ fontSize: 11, color: "#8fa1b3" }}>{predictNote}</div>
        </div>
        ...
        <div style={{ flex: 1, minHeight: 60 }}>
          ...
          {log.length === 0 && (
            <div style={{ fontSize: 12, color: "#5d6d7c" }}>Feed a nut to begin. Try large-grade nuts at 18 mm to see why per-grade batching is mandatory — then slide inside the derived window and watch the scatter vanish.</div>
          )}
          {log.map((e) => (
            <div key={e.id} style={{
              ...
              <span style={{ color: "#5d6d7c" }}>{GRADES[e.grade].label} w{e.dims ? e.dims.width : "?"} k{e.dims ? e.dims.kernel : "?"} @ {e.gap} mm</span>
            </div>
          ))}
        </div>

        <div style={{ fontSize: 10, color: "#46525f", lineHeight: 1.6, borderTop: "1px solid #26303b", paddingTop: 10 }}>
          C1 reconciliation: shell fractures when squeeze (width − gap) ≥ {DEFORM} mm — the transverse
          fracture deformation from Gallegos et al. (2013), replacing Rev B's unsourced 4 mm; kernel is
          crushed if the hard stop &lt; that nut's kernel Ø + {CLEAR} mm. The recommended window is DERIVED
          from these same rules, so the sim can no longer contradict itself. All dimensions are placeholders
          until the team's own survey and dissection data exist. Screw is a rigid stop — springs open only on overload.
        </div>
      </div>
    </div>
  );
